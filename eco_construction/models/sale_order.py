...
class SaleOrder(models.Model):
    ...
    move_boq = fields.Boolean(string='Move boq')
    create_project = fields.Boolean(string='Create Project')
    invoiced_amount = fields.Monetary(string="Invoiced Amount", store=True, compute='_compute_invoiced_amount')
    progeres_invoice_no = fields.Integer(string='Progeres Invoice Number')
    con_project_id = fields.Many2one('project.project', string='Project')
    con_project_name = fields.Char(string='Project Name')
    request_quota_line = fields.One2many(related='opportunity_id.request_quota_line')
    is_need_gm_approve = fields.Boolean(string='Need GM Approve')

    # ✅ NEW: Separate approval state
    ...
    @api.constrains('adv_percent', 'retention_percent')
    def check_total_percent(self):
        for rec in self:
            total_percent = rec.adv_percent + rec.retention_percent
    ...
    def action_om_approve(self):
        self.write({'approval_state': 'om_approve'})

    def action_om_reject(self):
        self.write({'approval_state': 'om_reject'})

    def action_sm_approve(self):
        if self.is_need_gm_approve:
            self.write({'approval_state': 'sm_approve'})
        else:
            self.write({'approval_state': 'approved'})

    def action_sm_reject(self):
        self.write({'approval_state': 'sm_reject'})

    def action_gm_approve(self):
        self.write({'approval_state': 'approved'})

    def action_gm_reject(self):
        self.write({'approval_state': 'gm_reject'})
    ...
    def action_confirm(self):
        # confirmation will only work if state is draft/sent (default behavior)
...
class SaleOrderLine(models.Model):
    ...
    def get_last_invoice_quantity(self, product_id):
        ...
        last_qty = last_invoice.invoice_line_ids.filtered(
            lambda x: x.product_id.id == product_id.id and x.display_type not in ('line_section', 'line_note')
        ).quantity or 0.0
        return last_qty
