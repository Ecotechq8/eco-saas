...
class DevSkipInstallment(models.Model):
    ...
    def action_send_request(self):
        ...
        if self.manager_id and self.manager_id.work_email:
            ...
            template_id = ir_model_data._xmlid_to_res_id('dev_hr_loan.dev_skip_dep_manager_approval')
            # get_object_reference('dev_hr_loan','dev_skip_dep_manager_approval')
    ...
    def approve_skip_installment(self):
        ...
        if email:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_to_res_id('dev_hr_loan.dev_skip_ins_hr_manager_request')
            # get_object_reference('dev_hr_loan','dev_skip_ins_hr_manager_request'))
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id)
    ...
    def dep_reject_skip_installment(self):
        if self.employee_id.work_email:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_to_res_id('dev_hr_loan.dep_manager_reject_skip_installment')
            # get_object_reference('dev_hr_loan','dep_manager_reject_skip_installment')
    ...
    def hr_reject_skip_installment(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_to_res_id('dev_hr_loan.hr_manager_reject_skip_installment')
            # get_object_reference('dev_hr_loan', 'hr_manager_reject_skip_installment')
    ...
    def confirm_skip_installment(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_to_res_id('dev_hr_loan.hr_manager_confirm_skip_installment')
            # get_object_reference('dev_hr_loan', 'hr_manager_confirm_skip_installment')
    ...
    def create_skip_account_journal_entry(self):
        ...
        lst.append((0, 0, {
            ...
            'partner_id': False,  # self.employee_id.address_home_id and self.employee_id.address_home_id.id or
            'name': self.name,
            'credit': self.installment_id.ins_interest or 0.0,
        }))
        credit_account = False
        if self.employee_id.user_id.partner_id and self.employee_id.user_id.partner_id.property_account_receivable_id:
            credit_account = self.employee_id.user_id.partner_id.property_account_receivable_id.id or False
        ...
        lst.append((0, 0, {
            ...
            'partner_id': False,  # self.employee_id.address_home_id and self.employee_id.address_home_id.id
            'name': '/',
