...
class AttendanceDeviceUser(models.Model):
    ...
    device_user_id = fields.Char(string='ID Number', size=8, help='The ID Number of the user/employee in the device storage', required=True, track_visibility='onchange')
    ...
    @api.constrains('device_user_id', 'device_id')
    def constrains_device_user_id_device_id(self):
        for r in self:
            if r.device_id and r.device_id.unique_uid:
                duplicate = self.search([('id', '!=', r.id), ('device_id', '=', r.device_id.id), ('device_user_id', '=', r.device_user_id)], limit=1)
                if duplicate:
                    raise UserError(_('The ID Number must be unique per Device!'
                                      ' A new user was being created/updated whose device_user_id and'
                                      ' device_id is the same as the existing one\'s (name: %s; device: %s; device_user_id: %s)')
                                      % (duplicate.name, duplicate.device_id.display_name, duplicate.device_user_id))
        ...
        for r in self:
            if r.del_user:
                try:
                    ...
                    r.device_id.delUser(r.uid, r.device_user_id)
    ...
    @api.onchange('device_user_id')
    def onchange_device_user_id(self):
        if self.device_user_id:
            self.device_user_id = self.env['to.base'].no_accent_vietnamese(self.device_user_id.replace(' ', ''))
    ...
    def _prepare_employee_data(self, barcode=None):
        barcode = barcode or self.device_user_id
        return {
            'name': self.name,
    ...
    def smart_find_employee(self):
        ...
        if self.employee_id:
            ...
            for employee in self.device_id.unmapped_employee_ids:
                if self.device_user_id == employee.barcode \
