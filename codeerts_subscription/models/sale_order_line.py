...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_invoiced(self):
        # v18 has NO _prepare_qty_invoiced hook (v19-only), so scope the recurring lines'
        # qty_invoiced to the CURRENT billing period here and delegate the rest to core. This
        # makes a recurring line reset to 0 each time the period rolls forward, so it re-bills.
        recurring = self.filtered(lambda l: l._is_recurring_subscription_line())
        super(SaleOrderLine, self - recurring)._compute_qty_invoiced()
            ...
            if period_start:
                for invoice_line in line._get_invoice_lines():
                    ...
                    invoice_qty = invoice_line.product_uom_id._compute_quantity(
                        invoice_line.quantity, line.product_uom, round=False)
