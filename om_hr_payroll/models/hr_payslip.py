...
class HrPayslipLine(models.Model):
    ...
    structure_id = fields.Many2one(
        'hr.payroll.structure', compute='_compute_structure_id',
        string='Salary Structure', store=True, index=True,
    )
    payslip_date_from = fields.Date(
        related='slip_id.date_from', string='Payslip Date From', store=True,
        index=True,
    )
    payslip_date_to = fields.Date(
        related='slip_id.date_to', string='Payslip Date To', store=True,
        index=True,
    )
    payslip_run_id = fields.Many2one(
        'hr.payslip.run', related='slip_id.payslip_run_id',
        string='Payslip Batch', store=True, index=True,
    )
    ...
    def _compute_structure_id(self):
        for line in self:
            line.structure_id = line.slip_id.struct_id or line.contract_id.struct_id
