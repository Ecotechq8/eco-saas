...
class PenaltyRequest(models.Model):
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('penalty.request')
    ...
    penalty_amount = fields.Float(string="Penalty Amount / Days", required=False, digits=(12, 3))
    penalty_type = fields.Selection(string="", selection=[('amount', 'Amount'),
                                                          ('days', 'Days'),
                                                          ('hours', 'Hours')], required=False)
    penalty_amount_amount = fields.Float(string="Penalty Amount", required=False, digits=(12, 3))
    penalty_amount_days = fields.Float(string="Penalty Days", required=False, digits=(12, 3))
    penalty_amount_hours = fields.Float(string="Penalty Hours", required=False, digits=(12, 3))
    ...
    day_value = fields.Float(string="", required=False, compute="_compute_day_value", digits=(12, 3))
    hour_value = fields.Float(string="", required=False, compute="_compute_hour_value", digits=(12, 3))

    @api.depends('employee_id', 'contract_id')
    def _compute_hour_value(self):
        for rec in self:
            rec.hour_value = 0.0
            if rec.employee_id and rec.contract_id:
                rec.hour_value = rec.contract_id.hour_value
    ...
    def _compute_day_value(self):
        for rec in self:
            rec.day_value = 0.0
            if rec.employee_id and rec.contract_id:
                rec.day_value = rec.contract_id.day_value
    ...
    def generate_payment_date(self):
        ...
        if self.penalty_type == 'hours':
            if not self.employee_id.contract_id:
                raise ValidationError(
                    "Sorry.. This Employee Dont Have Contract, you can choose Penalty type amount, or create contract for this employee")
            else:
                if self.penalty_amount_hours and self.employee_id:
                    self.penalty_ids = False

                    pen_hr_val = self.penalty_amount_hours * 60

                    minute_value = self.hour_value / 60
                    trasnfom_to_minutes_val = pen_hr_val * minute_value
                    amount = trasnfom_to_minutes_val
                    payment_time = {
                        'name': 'Penalty Request',
                        'date': date_start,
                        'hrs': self.penalty_amount_hours,
                        'amount': amount,
                        'penalty_id': self.id,
                        'paid': False,
                    }
...
class HrLoanDatePayment(models.Model):
    ...
    hrs = fields.Float(string="", required=False, )
