...
class HrPayslipInherit(models.Model):
    ...
    def onchange_sick_leave_employee(self):
        for item in self:
            if item.employee_id:
                input_lines = item.input_line_ids.filtered(lambda p: 'Rule' in p.input_type_id.name)
                ...
                if sick_leave_rules:
                    if sick_leave_lines:
                        for line in sick_leave_lines:
                            if line.rule_1 > 0.0:
                                input_type = self.env['hr.payslip.input.type'].search([('code', '=', "Rule1(0%)")])
                                if input_type:
                                    input_type = input_type.id
                                else:
                                    created_input_type = self.env['hr.payslip.input.type'].create(
                                        {'name': "Rule1(0%)", 'code': "Rule1"})
                                    input_type = created_input_type.id
                                if "Rule1" not in item.input_line_ids.mapped('name'):
                                    item.input_line_ids = [(0, 0, {
                                        'input_type_id': input_type,
                                        'name': "Rule1(0%)",
                                        'amount': item.contract_id.day_value * line.rule_1 *
                                                  sick_leave_rules.mapped('sick_leave_rules_ids')[0].deduction,
                                    })]

                            if line.rule_2 > 0.0:
                                input_type = self.env['hr.payslip.input.type'].search([('code', '=', "Rule2(25%)")])
                                if input_type:
                                    input_type = input_type.id
                                else:
                                    created_input_type = self.env['hr.payslip.input.type'].create(
                                        {'name': "Rule2(25%)", 'code': "Rule2"})
                                    input_type = created_input_type.id
                                if "Rule2" not in item.input_line_ids.mapped('name'):
                                    item.input_line_ids = [(0, 0, {
                                        'input_type_id': input_type,
                                        'name': "Rule2(25%)",
                                        'amount': item.contract_id.day_value * line.rule_2 *
                                                  sick_leave_rules.mapped('sick_leave_rules_ids')[1].deduction,
                                    })]

                            if line.rule_3 > 0.0:
                                input_type = self.env['hr.payslip.input.type'].search([('code', '=', "Rule3(50%)")])
                                if input_type:
                                    input_type = input_type.id
                                else:
                                    created_input_type = self.env['hr.payslip.input.type'].create(
                                        {'name': "Rule3(50%)", 'code': "Rule3"})
                                    input_type = created_input_type.id
                                if "Rule3" not in item.input_line_ids.mapped('name'):
                                    item.input_line_ids = [(0, 0, {
                                        'input_type_id': input_type,
                                        'name': "Rule3(50%)",
                                        'amount': item.contract_id.day_value * line.rule_3 *
                                                  sick_leave_rules.mapped('sick_leave_rules_ids')[2].deduction,
                                    })]

                            if line.rule_4 > 0.0:
                                input_type = self.env['hr.payslip.input.type'].search([('code', '=', "Rule4(75%)")])
                                if input_type:
                                    input_type = input_type.id
                                else:
                                    created_input_type = self.env['hr.payslip.input.type'].create(
                                        {'name': "Rule4(75%)", 'code': "Rule4"})
                                    input_type = created_input_type.id
                                if "Rule4" not in item.input_line_ids.mapped('name'):
                                    item.input_line_ids = [(0, 0, {
                                        'input_type_id': input_type,
                                        'name': "Rule4(75%)",
                                        'amount': item.contract_id.day_value * line.rule_4 *
                                                  sick_leave_rules.mapped('sick_leave_rules_ids')[3].deduction,
                                    })]

                            if line.rule_5 > 0.0:
                                input_type = self.env['hr.payslip.input.type'].search([('code', '=', "Rule5(100%)")])
                                if input_type:
                                    input_type = input_type.id
                                else:
                                    created_input_type = self.env['hr.payslip.input.type'].create(
                                        {'name': "Rule5(100%)", 'code': "Rule5"})
                                    input_type = created_input_type.id
                                if "Rule5" not in item.input_line_ids.mapped('name'):
                                    item.input_line_ids = [(0, 0, {
                                        'input_type_id': input_type,
                                        'name': "Rule5(100%)",
