...
class SaleOrder(models.Model):
    ...
    def _prepare_invoice(self):
        ...
        if invoice_vals:
            invoice_vals.update({'report_template_id': self.partner_invoice_id and self.partner_invoice_id.report_sale_template_id.id or False})
        return invoice_vals
    ...
    def print_quotation(self):
        ...
        if self.report_template_id or self.partner_id and self.partner_id.report_template_id or self.company_id and self.company_id.report_template_id:
            ...
            report.update({'report_name': 'sale.report_saleorder'})
            return report
        return res
    ...
    def _check_website_quote_installed(self):
        ...
        quote_installed = module_obj.search([('name', '=', 'website_quote'),('state', '=', 'installed')])
        res = {}
        if quote_installed:
            return True
        return False

    report_template_id1 = fields.Many2one('ir.actions.report.xml' , string="Sale Order Template", compute='_default_report_template1', help="Please select Template report for Sale Order", domain=[('model', '=', 'sale.order')])
    report_template_id = fields.Many2one('ir.actions.report.xml' , string="Sale Order Template", help="Please select Template report for Sale Order", domain=[('model', '=', 'sale.order')])
