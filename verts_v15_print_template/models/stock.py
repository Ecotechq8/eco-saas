...
class StockPicking(models.Model):
    ...
    def do_print_picking(self):
        ...
        if self.report_template_id or self.partner_id and self.partner_id.report_template_id or self.company_id and self.company_id.report_template_id:
            ...
            report.update({'report_name': 'stock.report_picking'})
            return report
        return res
    ...
    report_template_id1 = fields.Many2one('ir.actions.report.xml' , string="Picking Template", compute='_default_report_template1', help="Please select Template report for Picking", domain=[('model', '=', 'stock.picking')])
    report_template_id = fields.Many2one('ir.actions.report.xml' , string="Picking Template", help="Please select Template report for Picking", domain=[('model', '=', 'stock.picking')])
