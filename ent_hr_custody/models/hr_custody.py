...
class HrCustody(models.Model):
    ...
    mobile_sim_card = fields.Boolean(string='Mobile SIM Card')
    mobile_no = fields.Char(string='Mobile No')
    pin_no = fields.Char(string='Pin No')

    mobile_handset = fields.Boolean(string='Mobile Handset')
    handset_brand = fields.Char(string='Brand')
    handset_serial_no = fields.Char(string='Serial No')

    car = fields.Boolean(string='Car')
    car_brand = fields.Char(string='Brand')
    year_of_model = fields.Char(string='Year of Model')

    laptop = fields.Boolean(string='Laptop')
    lap_brand = fields.Char(string='Brand')
    lap_serial_no = fields.Char(string='Serial No')


    standalone_printer = fields.Boolean(string='Stand-Alone Printer')
    standalone_brand = fields.Char(string='Brand')
    standalone_no = fields.Char(string='Serial No')

    others_1 = fields.Boolean(string='Others 1')
    item_name = fields.Char(string='Item Name')
    details = fields.Text(string='Details')

    job_title = fields.Many2one('hr.job', related='employee.job_id', string='Job Title', store=True)
    type_id = fields.Many2one('hr.operation.type', string='Operation Type', required=True)

    identification_no = fields.Char(related='employee.identification_id', string='Identification No', store=True)
    nationality = fields.Char(related='employee.country_id.name', string='Nationality', store=True)
    manager_id = fields.Many2one(related='employee.parent_id', string='Manager', store=True)

    department_id = fields.Many2one(related='employee.department_id', string='Department', store=True)
    date_of_birth = fields.Date(related='employee.birthday', string='Date of Birth', store=True)
    ...
    new_or_used = fields.Selection([
        ('new', 'New'),
        ('used', 'Used')
    ], string='The item received is')
        ...
        for record in self:
            ...
            if record.employee:
                contract = self.env['hr.contract'].search([
                    ...
                    ('state', '=', 'open')  # Ensure the contract is a running one
                ], limit=1)
