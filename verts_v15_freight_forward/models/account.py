...
class AccountMove(models.Model):
    ...
    def _onchange_invoice_line_ids(self):
        ...
        if self.currency_id.id != self.company_currency_id.id:
            self.with_context(manual_currency_rate=self.cur_rate)._onchange_currency()
    ...
    def _onchange_currency(self):
        for val in self.with_context(manual_currency_rate=self.cur_rate):
            ...
            if val.is_invoice(include_receipts=True):
                for line in val._get_lines_onchange_currency():
            ...
            val._recompute_dynamic_lines(recompute_tax_base_amount=True)
