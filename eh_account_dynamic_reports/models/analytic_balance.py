...
class EhAnalyticBalanceHandler(models.AbstractModel):
    ...
    def compute(self, options):
        ...
        for (plan_id, plan_name), plan_rows in sorted(
            ...
            lines.append(self._section_header_line(
                plan_name or "Unassigned plan", section_id=section_id,
            ))
            section_total = 0.0
            ...
            lines.append(self._section_total_line(
                "Total %s" % (plan_name or "Unassigned"),
        ...
        lines.append(self._computed_line(
            'analytic_total', "Net analytic impact",
            round(grand_total, 2), kind='computed_total',
        ))

        return {
            'columns': self._build_two_column_layout(
                label_name="Analytic account",
                amount_name="Net amount",
            ),
            'lines': lines,
            'totals': {
    ...
    @api.model
    def _build_two_column_layout(self, label_name="Analytic account",
                                  amount_name="Net amount"):
        return [
            {'expression_label': 'analytic', 'name': label_name,
             'figure_type': 'string'},
    ...
    def _fetch_analytic_rows(
        ...
        query.where_raw(SQL("aml.analytic_distribution IS NOT NULL"))
        # Apply standard option filters EXCEPT analytic ones (we want
        # every analytic account to appear, not be filtered out).
        ...
        # Account-type filter from options (income/expense by default
        # so the report covers operational performance per analytic;
        # callers can override via options['analytic_account_types']).
        types = options.get('analytic_account_types') or (
            'income', 'income_other',
            'expense', 'expense_depreciation', 'expense_direct_cost',
        )
        query.where_account_types(types)

        # Cross-join the jsonb distribution keys: jsonb_each_text yields
        # one (key, percentage) pair per analytic account allocated. We
        # multiply the line balance by percentage/100 and sum.
        query.select(SQL(
            "(jsonb_each_text(aml.analytic_distribution)).key::int"
        ), 'analytic_id')
        query.select(SQL(
            "SUM(aml.balance * "
            "((jsonb_each_text(aml.analytic_distribution)).value::float) "
            "/ 100.0)"
        ), 'amount')
        query.group_by(SQL(
            "(jsonb_each_text(aml.analytic_distribution)).key::int"
        ...
        accessible = Analytic.browse(ids).filtered(
            lambda a: a.check_access_rights('read', raise_exception=False)
        )
        ...
        accessible_plans = Plan.browse(plan_ids).filtered(
            lambda p: p.check_access_rights('read', raise_exception=False)
        )
