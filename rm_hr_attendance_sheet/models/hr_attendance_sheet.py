...
class AttendanceSheet(models.Model):
    ...
    def _compute_sheet_total(self):
        ...
        for sheet in self:
            ...
            # Compute Attendance
            if sheet.line_ids:
                attendance_lines = sheet.line_ids.filtered(lambda l: l.worked_hours > 0)  # and l.status == False
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            if not tz:
                raise exceptions.Warning("Please add time zone for employee : %s" % emp.name)
            calendar_id = emp.contract_id.resource_calendar_id if not att_sheet.att_policy_id.is_flexible else False
            if not calendar_id and not att_sheet.att_policy_id.is_flexible:
            ...
            for day in all_dates:
                ...
                # Use this for night shift.
                if calendar_id:
                    if 'afternoon' in list(set(calendar_id.attendance_ids.mapped(
                            'day_period'))) and att_sheet.att_policy_id.is_night_shift:
                        day_end = day_start + timedelta(days=1)
                        day_end = day_end.replace(hour=11, minute=59, second=59)
                day_str = str(day.weekday())
                date = day.strftime('%Y-%m-%d')
                work_intervals = att_sheet._get_work_intervals(calendar_id, day_start, day_end,
                                                               tz) if not att_sheet.att_policy_id.is_flexible else []
                ...
                if work_intervals:
                    ...
                    if att_sheet.att_policy_id.is_flexible:
                        if attendance_intervals:
                            for attendance_interval in attendance_intervals:
                                overtime = attendance_interval[1] - attendance_interval[0]
                                float_overtime = overtime.total_seconds() / 3600
                                ...
                                worked_hours = attendance_interval[1] - attendance_interval[0]
                                float_worked_hours = worked_hours.total_seconds() / 3600
                                plan_sign_in, plan_sign_out = 0, 0

                                diff_time = att_sheet.att_policy_id.number_of_hours_per_day - float_worked_hours
                                act_float_diff = float_diff = diff_time if diff_time > 0.0 else 0.0

                                values = {
                                    'date': date,
                                    'day': day_str,
                                    'pl_sign_in': plan_sign_in if plan_sign_in > 0.0 else 0.0,
                                    'pl_sign_out': plan_sign_out if plan_sign_out > 0.0 else 0.0,
                                    'ac_sign_in': self._get_float_from_time(ac_sign_in),
                                    'ac_sign_out': self._get_float_from_time(ac_sign_out),
                                    'overtime': act_float_overtime if act_float_overtime > 0.0 else 0.0,
                                    'act_overtime': act_float_overtime if act_float_overtime > 0.0 else 0.0,
                                    'worked_hours': float_worked_hours,
                                    'late_in': 0.0,
                                    'act_late_in': 0.0,
                                    'diff_time': float_diff,
                            ...
                            status = 'ph' if public_holiday else 'leave' if leaves else 'ab'
                            values = {
                                'date': date,
                                'day': day_str,
                                'att_sheet_id': self.id,
                                'diff_time': att_sheet.att_policy_id.number_of_hours_per_day if status not in (
                                    'leave', 'ph') else 0.0,
                                'status': status,
                                'note': ""
                            }
                        ...
                        if attendance_intervals:
                            ...
                            for attendance_interval in attendance_intervals:
                                ...
                                if float_overtime <= overtime_policy['wd_after']:
                                    ...
                                    act_float_overtime = (float_overtime - calendar_id.hours_per_day)
                                    float_overtime = act_float_overtime * overtime_policy['wd_rate']

                                ac_sign_in = pytz.utc.localize(attendance_interval[0]).astimezone(tz)
                                ac_sign_out = pytz.utc.localize(attendance_interval[1]).astimezone(tz)
                                # float_ac_sign_out = self._get_float_from_time(ac_sign_out)
                                worked_hours = attendance_interval[1] - attendance_interval[0]
                                float_worked_hours = worked_hours.total_seconds() / 3600
                                plan_sign_in, plan_sign_out = 0, 0
                                for line in calendar_id.attendance_ids:
                                    kay_val_dict = dict(line._fields['dayofweek'].selection)
                                    for key, val in kay_val_dict.items():
                                        if val == datetime.strftime(
                                                datetime.strptime(str(attendance_interval[0]), '%Y-%m-%d %H:%M:%S'),
                                                '%A'):
                                            plan_sign_in = line.hour_from
                                        elif val == datetime.strftime(
                                                datetime.strptime(str(attendance_interval[1]), '%Y-%m-%d %H:%M:%S'),
                                                '%A'):
                                            plan_sign_out = line.hour_to

                                status = "weekend"
                                note = _("working in weekend") if ((overtime.total_seconds() / 3600) ==
                                                                   float_worked_hours) else _(
                                    "overtime out of work intervals")

                                if plan_sign_in and plan_sign_out:
                                    if day_str in calendar_id.attendance_ids.mapped('dayofweek'):
                                        status, note = False, False
                                    if float_overtime > calendar_id.hours_per_day:
                                        float_overtime -= calendar_id.hours_per_day
                                    elif float_overtime == calendar_id.hours_per_day and note == "overtime out of work intervals":
                                        float_overtime = calendar_id.hours_per_day
                                    elif status == 'weekend':
                                    ...
                                    if public_holiday:
                                        act_float_overtime = (
                                                overtime.total_seconds() / 3600 - overtime_policy['ph_after'])
                                        float_overtime = (overtime.total_seconds() / 3600 - overtime_policy[
                                            'ph_after']) * overtime_policy['ph_rate']
                                        status = 'ph'
                                        note = _("working on Public Holiday")
                                    else:
                                        if not att_sheet.att_policy_id.is_night_shift or float_overtime < 0.0:
                                            act_float_overtime, float_overtime = 0.0, 0.0

                                diff_time = timedelta(hours=00, minutes=00, seconds=00)
                                late_in = timedelta(hours=00, minutes=00, seconds=00)

                                float_pl_in = self._get_float_from_time(attendance_interval[0] + timedelta(hours=3))
                                float_pl_out = self._get_float_from_time(attendance_interval[1] + timedelta(hours=3))
                                if float_pl_in > plan_sign_in:
                                    late_in = float_pl_in - plan_sign_in
                                if float_pl_out < plan_sign_out:
                                    diff_time = plan_sign_out - float_pl_out

                                float_late = act_float_late = late_in
                                # policy_late, late_cnt = policy_id.get_late(float_late, late_cnt)
                                # float_diff = diff_time.total_seconds() / 3600
                                act_float_diff = float_diff = diff_time
                            ...
                            for line in calendar_id.attendance_ids:
                                ...
                                plan_sign_out = line.hour_to
                            status = 'ph' if public_holiday else 'leave' if leaves else 'ab'
                            if day_str not in calendar_id.attendance_ids.mapped('dayofweek'):
                                status = 'weekend'

                            if att_sheet.att_policy_id.is_night_shift:
                                diff_time = calendar_id.hours_per_day
                            else:
                                diff_time = att_sheet.att_policy_id.number_of_hours_per_day
                            ...
                            values = {
                                ...
                                'day': day_str,
                                'att_sheet_id': self.id,
                                'diff_time': diff_time if status not in ('leave', 'ph', 'weekend') else 0.0,
                                'status': status,
                                'note': ""
                            }
                            att_line.create(values)
    ...
    def action_create_payslip(self):
        ...
        for sheet in self:
            ...
            new_payslip = payslip_obj.new({
                ...
                'struct_id': contracts[0].struct_id.id if contracts[0].struct_id else False
            })
    ...
    def _get_workday_lines(self):
        ...
        if not contract:
            raise ValidationError("No contract found for employee")
        ...
        return overtime + late + absence + difftime
