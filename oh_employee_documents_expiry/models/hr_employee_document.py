...
class HrEmployeeDocument(models.Model):
    ...
    def _onchange_document_type_id(self):
        for item in self:
            if item.document_type_id:
                item.notification_type = item.document_type_id.notification_type
                item.before_days = item.document_type_id.before_days
    ...
    def mail_reminder(self):
        ...
        for record in self.search([('expiry_date', '!=', False)]):
            ...
            is_notification_day = any([record.notification_type == 'single' and is_expiry_today,
                                       record.notification_type == 'multi'
                                       and (fields.Date.today() == fields.Date.
                                            from_string(record.expiry_date) - days_before or is_expiry_today),
                                       record.notification_type == 'everyday'
                                       and fields.Date.today() >= fields.Date.from_string(
                                           record.expiry_date) - days_before,
                                       record.notification_type == 'everyday_after'
                                       and fields.Date.today() <=
                                       fields.Date.from_string(record.expiry_date) + days_before,
                                       not record.notification_type and fields.Date.today() == fields.Date.
                                      from_string(record.expiry_date) - timedelta(days=7)])
            if is_notification_day:
                ...
                subject = _('Document-%s Expired On %s') % (document_name, expiry_date_str)
                email_notify_group = self.env.ref('oh_employee_documents_expiry.group_employee_expiry_mail_send').id
                users = self.env['res.users'].sudo().search([('groups_id', 'in', [email_notify_group])])
                print(users)
                for user in users:
                    todos = {
                        'res_id': record.id,
                        'res_model_id': self.env['ir.model'].search([('model', '=', 'hr.employee.document')]).id,
                        'user_id': user.id,
                        'summary': subject,
                        'note': mail_content,
